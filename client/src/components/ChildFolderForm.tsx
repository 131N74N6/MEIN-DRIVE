...
import type { ChildFolderFormIntrf } from "../client_models/folder.client_model";

export default function ChildFolderForm(props: ChildFolderFormIntrf) {
    function makeFolder(event: React.FormEvent) {
        event.preventDefault();
        props.submit_folder.mutate(props.parent_folder_id);
    }

    return (
        <div className='flex justify-center items-center fixed inset-0 z-20 border bg-[rgba(0,0,0,0.66)] p-3'>
            <form onSubmit={makeFolder} className="bg-white flex flex-col gap-[1rem] p-[1rem] w-[300px] border border-white">
                <button 
